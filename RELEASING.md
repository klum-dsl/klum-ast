...
./gradlew renderVersionedDocumentation verifyRenderedDocumentationSite \
  -PdocumentationRevision="$revision" \
  -PdocumentationRendererRevision="$revision" \
  -PdocumentationVersion=4.0.0-tracer \
  -PdocumentationStatus=tracer \
  -PdocumentationBrandingManifest=docs/branding/season-4-klumast.json \
  -PdocumentationObjectDirectory=. \
  -PdocumentationOutputDirectory=build/versioned-documentation/local-review
```

To review the same generated bytes in a browser, replace
`verifyRenderedDocumentationSite` with `previewRenderedDocumentation`; open the printed
`http://127.0.0.1:<port>/klum-ast/` URL and stop the task with Ctrl-C. The automated check serves
the site under that real repository base path and crawls internal pages, assets, directory indexes,
and heading fragments. It does not contact GitHub or publish anything.
