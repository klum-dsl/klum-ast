...
Standalone root factories still return completed models. A custom factory or converter that creates a completed model remains suitable only where that model is the root result or an aggregation `LINK` target. The Builder-producing converter protocol is recorded in [ADR 0004](https://github.com/klum-dsl/klum-ast/blob/master/docs/adr/0004-template-recipe-companions-and-builder-converters.md); until that protocol is available, owned DSL Object converters must use the generated child construction methods rather than a nested factory.
