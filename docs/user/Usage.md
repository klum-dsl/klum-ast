...
For a multi-project build, use `./gradlew generateAllKlumDslSourceMirrors` to refresh every Schema project's mirror, including Layer 3 `api` and `schema` projects. The root task coordinates only; mirrors remain owned by their Schema projects. The refresh also materializes one root-owned GDSL resource directory and registers that same directory with every Schema module so IntelliJ discovers the static `Create` contributor without duplicate module artifacts. GDSL and mirrors provide completion metadata and are not build or publication inputs. [AnnoDoc Support for IntelliJ IDEA](https://github.com/blackbuild/annodoc-intellij)
