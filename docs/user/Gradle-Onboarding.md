...
It refreshes every project that applies the Schema plugin, including both `api` and `schema` projects in a Layer 3 layout. The generated `Foo_DSL` mirrors provide IntelliJ completion only. After a refresh, the packaged IntelliJ contributor resolves source-level `Foo.Create` as the matching read-only `Foo_DSL.Factory` and `Foo.Template` as the matching read-only `Foo_DSL.Template`, so completion continues through their public methods. It contributes nothing for ordinary classes or when the public namespace cannot be resolved. Mirrors are not compiled, packaged, published, or added to downstream classpaths. The aggregate has no generated payload of its own; each Schema project remains the owner of its mirror task.
