...
| A manual configurator shadows a field in a factory map | Map keys intentionally call the same-named Builder method before considering storage. A non-void override can be mistaken for an ordinary helper. | Keep a setter-like return (`void`, the field value, or its Builder) to receive a warning, or rename the helper. Use `setX` in the map when direct field assignment is intended. An incompatible non-void return is a compilation error. |
