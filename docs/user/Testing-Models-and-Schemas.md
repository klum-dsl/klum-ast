...
For a test that needs materialized Templates during `setup`, the feature body, and `cleanup`, add the separately published
test-only support artifact alongside the ordinary runtime. Keep the versions aligned with the KlumAST BOM:
