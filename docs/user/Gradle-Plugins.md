...
- registers `createKlumDslSourceMirrors` to refresh AnnoDocimal source mirrors for generated `Foo_DSL` support
  ...
  `./gradlew generateAllKlumDslSourceMirrors` aggregate instead; it lazily runs each participating Schema project's task,
  including `api` and `schema` in a Layer 3 layout. It compiles the real generated interfaces first when necessary and
  exposes the mirror directory to IntelliJ as a generated source root. Each refresh first materializes the packaged GDSL
  contributors once below the root build directory and registers that one generated resource root with every Schema module,
  so IntelliJ discovers the contributors without module-local copies. The aggregate has no payload of its own; GDSL and
  mirrors are IDE metadata and are not compiled, packaged, published, or added to downstream classpaths.
