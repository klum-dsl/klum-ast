...
| --- | --- | --- |
| `KlumObjectSupport.of`, `getObject`, `getConstructionPath`, `getModelPath`, `getStructure`, `getValidation`, and public members of `Structure`/`Validation` | Client entrypoint; #390, #549 | Export the runtime API package. Source and binary compatible throughout 4.x. `getConstructionPath(): String` is the sole construction-string getter. `Validation.getResult()` reads the target's result; `Validation.getSubtreeResults()` reads the target plus owned subtree. |
