...
| `com.blackbuild.klum.ast.Builder` namespace with nested runtime-retained `Method`, `Query`, `Input`, and `Result`; deprecated `Mutator` source bridge | Schema authoring entrypoint; #689 / #651 / #650 | Export the annotation API package. `Method` is Builder-only, `Query` is retained on the Model and projected, and `Input`/`Result` select individual projected signature positions. `Mutator` remains a 4.1 source-compatible spelling promoted to `Builder.Method`; the outer namespace is not an annotation or generated Builder type. |
| Generated `Foo_DSL.Factory extends BuilderFactoryProvider<Foo, Foo_DSL.Builder<Foo>>`, nested `Foo_DSL.Factory.Template`, `Foo_DSL.TemplateScope`, `Foo_DSL.Builder`, collection/cluster factory interfaces; `Foo.Create`, `Foo.Create.Template`, `Foo.Template`, and `Foo.Create.AsBuilder()` plus typed polymorphic relationship overloads and explicitly selected Builder capability methods | Generated hook; #729, #394, #431, #474, #620, #710, #737, #689 | Export runtime types referenced by generated descriptors. Generated interfaces may be named in signatures, never implemented or subclassed. Explicit `Builder.Query`/`Method` operations and `Builder.Input`/`Result` positions use exact public Builder types; unselected Model methods remain absent. `Foo.Create.Template` is the static final root-Template factory property; `Foo.Template` is typed as `Foo_DSL.TemplateScope`, the scoped-application handler whose deprecated creation aliases are compatibility members. The #620 overloads preserve the dynamic Class selector and expose the exact selected public Builder without starting a root lifecycle. |
| `KlumBuilder<T>`; `KlumFactory.getModelType()`; `KlumFactory.BuilderFactoryProvider<T, B>` with `getModelType()`, `AsBuilder()`, `isModelOrBuilder(Object)`, `isBuilder(Object)`, and `narrowBuilder(Object)`; `BuilderFactory.getModelType()`; and the remaining `KlumFactory` Builder-factory descriptors | Generated hook; #729 / #394 / #431 / #620 / #648 | Runtime API export required for emitted bytecode; compatible in 4.x after intentional 3.x-to-4.0 recompilation. The provider exposes the selected model type, explicit active-session `AsBuilder()` operation, state predicates, and exact identity-preserving narrowing. Narrowing performs no creation or lifecycle transition; `KlumBuilder<T>` remains zero-operation. |
...
implementations, #620's exact typed relationship Factory-provider signatures, and #689's combined query, input/result,
factory-narrowing, direct/inherited, and mirror contract. `JpmsPackageBoundaryTest` additionally
