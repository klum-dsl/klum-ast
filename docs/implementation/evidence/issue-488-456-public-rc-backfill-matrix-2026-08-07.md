...
4. Only after that separate approval, create the exact annotated tag and matching GitHub release for the one reviewed identity, then dispatch the unified workflow once. It first validates that record, creates its new proof internally, and only then enters protected documentation promotion. Stop on the first mismatch and retain the incident record. Do not batch, replace, supersede, or repair versions in place.
