...
JP-ABI-1, JP-ABI-2, JP-ABI-3, and this JP-ABI-4 breadcrumb slice migrate the
Builder, model-state, Materialization, Template, and breadcrumb entries below
to the generated-runtime bridge. The named-module tracer must regenerate the
full representative output after the remaining Cluster and omitted-projection
