...
- **Content representation:** authored pages remain plain Markdown. Pinned `commonmark-java` core and GFM-table components in `buildSrc` produce complete static HTML with a small repository-owned shell/CSS and no runtime Jekyll, Node, browser framework, CDN, or required JavaScript. Existing sidebar/footer files remain input fragments; local images and future build-time diagram outputs are copied assets, while authored raw HTML is escaped and unsafe URLs are sanitized.
...
- **Deterministic tree:** `Home.md` becomes `index.html`; each other Markdown page becomes `path/Page/index.html`. Relative extensionless links, GitHub-wiki links, GitHub-compatible heading fragments, local assets, API pages, and Javadocs are resolved before hashing. The canonical UTF-8 `site-manifest.json` records renderer contract/revision, source identity/tree hash, version/status, branding/assets, Javadoc inputs, and every exact-tree output hash except itself. It omits authored Markdown, timestamps, host/user data, absolute paths, secrets, cache paths, and telemetry.
- **Verification:** hermetic Git fixtures cover current and historical source roots, status chrome, local branding/assets, raw-HTML escaping, tables, duplicate/Unicode anchors, six isolated API bases, extensionless deep links, and repeated byte-identical output. A JDK HTTP server and crawler exercise the complete fixture under `/klum-ast/` and verify internal pages, assets, directory indexes, and fragments without browser/runtime dependencies. Negative fixtures reject dirty or abbreviated/unresolvable revision input, wrong root, malformed branding, duplicate output path, invalid status/alias state, failed Javadocs, and an existing immutable target. The credential-free tracer has no publish, deploy, credential, or alias dependency; the protected stage excludes artifact publication and alias promotion.
...
- **Protected Pages handoff:** a #456-owned, separately permissioned Pages workflow/job runs before #488 artifact publication. It independently validates stage/version/SHA, requires the post-rehearsal `DOCUMENTATION_PAGES_READY` interlock and `.nojekyll` ledger, commits and reads back the immutable `gh-pages` state, then uploads/deploys that exact state with the official Pages actions. It returns only validated identity/evidence (`stage`, `version`, `sha`, exact path, site-manifest SHA-256, gh-pages commit, Pages run identity, and page URL). It has no artifact credentials or release/tag/alias authority. #488 rechecks the returned identity before publication and its public proof remains the sole alias-promotion input.

## Dependency-ordered tracer and implementation slices

### VD-1 — Pin the site contract and deterministic renderer

**Work.** In the same change, move the current 4.x authoring tree from wiki/ to docs/user/, update its relative links/navigation and the contributor instructions, and add a local renderer with explicit revision, version, status, and output-directory inputs. The renderer selects docs/user/ for 4.x and does not treat wiki/ as a second current source. Replace or make the old gitPublish path fail closed so it cannot publish the prior mutable tree. Implement deterministic static HTML, page chrome for every exact version, extensionless directory URLs, local assets, and /archive/ discovery. Accept a branding manifest with Season identity, logo asset, alt text, and digest, then copy the logo to the exact tree and record it in the site manifest. Implement immutable-tree checks and a machine-readable site manifest containing source-root/revision/tree hash, renderer contract/revision, version/status, branding manifest/digest, Javadoc inputs/checksums, output hashes, and no secrets.
...
**Work.** Add protected Pages task/workflow accepting exact candidate/final stage, version, and full SHA; recompute Nebula version, prove SHA is on master, and reject mismatch, used path/tag, or malformed stage. Candidate rendering captures its branding manifest; final rendering additionally requires the branding-owner approval attached to that exact manifest. Render only that revision, run the local HTTP/link proof, commit the immutable unlisted pending tree plus manifest to the `gh-pages` ledger, read it back, and deploy that exact ledger through the official Pages artifact/deploy actions before #488 artifacts. Fail closed until the disposable empty-site rehearsal has been removed and `DOCUMENTATION_PAGES_READY=true` records the verified fresh `.nojekyll` ledger and protected environments. Hand the verified values to #488 orchestration without copying artifact credentials.

**Acceptance.** Tests reject mismatched stage/version/SHA, off-master SHA, existing immutable path, malformed identity, missing manifest, absent readiness/`.nojekyll` bootstrap, unapproved or changed final branding manifest, and artifact publication before the read-back and protected Pages deployment succeed. Pending output cannot be confused with preview/stable.
...
VD-4 + VD-5 ─> #488 protected artifact publication ─> isolated public-availability proof ─> maintainer tag/release
          └────────────────────────────────────────────────────────────────────────────────────────────> #488 REL-2 public-release verification dispatch ─> credential-free proof ─> #456 promotion (VD-6 aliases) ─> independent consumer
```

For a real RC/final, #456 validates the same values and produces only unlisted pending documentation. #488 owns authorization, signing/registry and plugin publication, public resolve-back, the maintainer-visible tag/release record validation, external proof, and incident recovery. The documentation stage receives the parent workflow's retained proof result, not a licence to infer one or a manually copied run identity. The proof-gated promotion workflow remains #456-owned and is invoked only after #488's unified verification validates the manually created annotated tag and matching release; it has neither artifact nor release-record credentials. A documentation-only correction after an accepted RC follows ADR 0012; source, release-configuration, or workflow change needs the next RC.
