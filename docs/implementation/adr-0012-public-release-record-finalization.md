...
The finalizer uses a protected `release-record` environment with human approval. Its ambient `GITHUB_TOKEN` remains read-only; only after its immutable rechecks does it mint a dedicated release-record GitHub App token. That App has only `Contents: write` and `Workflows: write`, which is necessary to tag an accepted source that itself changes workflow files. It has no artifact, signing, registry, or Pages-writer credentials. `release.yml` remains `contents: read`, so publication cannot create tags or release records. Source selection always checks out and verifies the supplied full SHA, even when workflow code on `master` has advanced.
