...
| #390 — ADR 0006 completed Object support | OS-1/OS-2 are implemented, but 4.0 still needs the pre-freeze `getConstructionPath()` correction, terminology/migration closure, and final API fixtures. | [#435 OS-1](https://github.com/klum-dsl/klum-ast/issues/435), OS-2, and OS-3: root/subtree facade, construction/structural path distinction, structure, stored validation, proxy lockdown, serialization, and Java-first docs. | ADR 0010 has completed #467's naming decision; #468 freezes the delivered public member. Provenance/lineage remains #402. |
