...
| #456 — versioned documentation and Javadocs | A 4.0 release cannot let 3.0.1 users read mutable Builder-first guidance or publish artifacts without matching immutable documentation/API reference. | ADR 0013's credential-free tracer renders v3.0.1 plus a fixed 4.0 SHA, six isolated module API trees, selector/deep-link/wiki-stub fixtures, and source manifests. The protected Pages stage validates the same stage/version/master SHA, deploys an unlisted immutable pending snapshot before #488 artifacts, and advances aliases only after #488 public proof. | #456 owns renderer/site/tracer/Pages/alias implementation. #488 owns authorization, credentials, artifact publication, proof, and recovery. AnnoDocimal #71 and KlumCast #47 are linked local counterparts only. |
