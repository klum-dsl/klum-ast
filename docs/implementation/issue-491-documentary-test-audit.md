...
| `Converters.md` — Field-Based Converters, Factory Method Converters, and Factory Classes | #148; #198, #243, #300, #319 for focused boundary coverage | `ConverterSpec`, `ConvertersDocumentaryTest` | Aligned as one dedicated documentary cohort. The `@Converters` customization matrix is an API-reference path, while overload, visibility, generic, ambiguity, collection-factory, and script boundaries remain focused coverage rather than separate tutorials. |
...
- `Converters.md` — **Customization** links to the complete `@Converters` API/Javadoc option matrix. It does not present
  one settled tutorial path; `includeMethods`, `excludeMethods`, `excludeDefaultPrefixes`, and `includeConstructors`
  remain focused discovery/selection coverage in `ConverterSpec` rather than an artificial documentary example.
