...
Completed DSL Objects need supported access to provenance, validation, and composition structure without exposing their
...
`getBreadcrumbPath()` is construction provenance and `getModelPath()` is structural location. They remain direct methods;
future generation/event logs from #402 may justify another grouped helper but do not pre-empt this API.
...
- Provenance, structure, and validation are supported without leaking companion identity or metadata.
