...
Implementation is complete only when a separately published support coordinate resolves with the normal runtime through the BOM; its public/Javadoc/JPMS surface contains only the scope type; Java and Spock use restore exact nested state after success and failure; the existing Template behavior remains covered; and clean external Groovy 3, 4, and 5 consumers resolve and execute the artifact without internal imports or cross-lane test output.
