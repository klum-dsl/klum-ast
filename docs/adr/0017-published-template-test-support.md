...
This is a test-support API, not a new production-runtime construction API. Its Javadocs and user guidance keep it in a test dependency configuration. The support artifact has an `implementation` dependency on `klum-ast-runtime` for execution. A consumer that applies either the Schema or Model plugin receives the support coordinate automatically in `testImplementation`, alongside the plugin's existing test baseline; it is never added to a production configuration. Direct Java/Groovy consumers that apply neither plugin declare the BOM, normal runtime, and test-support coordinate together so the supported runtime remains explicit. Plugin provision is independent of whether the consumer keeps the default Spock support: the scope is also usable from Java/JUnit tests. The BOM constrains the new coordinate just as it constrains every Java-library subproject.
...
- Schema and Model modules receive the support artifact on their test classpath without a manual coordinate; production classpaths and published metadata remain unchanged.
...
Implementation is complete only when a separately published support coordinate resolves with the normal runtime through the BOM and is automatically available in Schema and Model modules' `testImplementation` configurations; its public/Javadoc/JPMS surface contains only the scope type; Java and Spock use restore exact nested state after success and failure; the existing Template behavior remains covered; and clean external Groovy 3, 4, and 5 consumers resolve and execute the artifact without internal imports or cross-lane test output.
