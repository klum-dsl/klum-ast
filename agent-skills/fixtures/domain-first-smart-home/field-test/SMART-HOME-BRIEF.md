...
The team has one generic client and one Model Writer who configures a sample apartment. Heated rooms require a thermostat, the Kitchen requires a smoke detector, and Window sensors are optional. Labels are stable Schema defaults. The client needs generic areas and Windows, but must not need the concrete room Schema classes or know which provider configures a device.
