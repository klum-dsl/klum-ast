...
7. In IntelliJ IDEA, import the project as a Gradle project. After Schema changes, run `./gradlew createKlumDslSourceMirrors` in a single-project build or `./gradlew generateAllKlumDslSourceMirrors` in a multi-project build, then reload the Gradle project. The root aggregate refreshes every Schema project, including Layer 3 `api` and `schema` projects, without owning a mirror payload. The refresh materializes the packaged GDSL contributors once below the root build directory and registers the same resource directory with every Schema module. GDSL and mirrors provide completion metadata only: they are not compiled, packaged, published, or downstream inputs. Compiled Quick Documentation is a separate AnnoDoc Support concern.
