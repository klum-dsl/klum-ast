...
- One manually dispatched Verify public release run now retains its credential-free public proof first, then binds the proof internally to protected documentation promotion and the separately approved `release-record` finalizer. Maintainers enter only stage, version, and SHA; RCs can advance `/preview/`, while only finals can advance `/stable/` and a maintained-line alias such as `/4.0/`. The artifact publisher retains no contents-write authority ([#488](https://github.com/klum-dsl/klum-ast/issues/488), [#456](https://github.com/klum-dsl/klum-ast/issues/456)).
