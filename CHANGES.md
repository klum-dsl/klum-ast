...
- IntelliJ support now materializes the packaged GDSL contributors once in a root-owned, IDE-only directory and registers
  ...
  version-sensitive internal raw-member hook, validated with IntelliJ IDEA 2026.2; it changes neither bytecode nor
  read-only runtime semantics. Ordinary classes and unresolved support namespaces receive no synthetic property, and the
  retained polymorphic closure contributor delegates through the public `Foo_DSL.Builder` contract. Neither the GDSL root
  nor the mirrors are compilation, publication, or downstream inputs ([#703](https://github.com/klum-dsl/klum-ast/issues/703)).
