...
- After #488's credential-free public proof, a separate protected manual documentation workflow validates the immutable proof and matching pending evidence, re-renders a public exact tree, records the promotion, and advances only eligible aliases. RCs can advance `/preview/`; only finals can advance `/stable/` and a maintained-line alias such as `/4.0/` ([#456](https://github.com/klum-dsl/klum-ast/issues/456)).
- Publicly proven releases now finalize their immutable annotated tag, matching GitHub release record, exact public documentation, and eligible aliases through a separate human-approved `release-record` authority before independent external-consumer validation. The artifact publisher retains no contents-write authority ([#488](https://github.com/klum-dsl/klum-ast/issues/488), [#456](https://github.com/klum-dsl/klum-ast/issues/456)).
